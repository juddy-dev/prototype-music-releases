...
function EditArtistLabel(props) {
  const { data, label, onClose } = props;
  ...
  const [ withSpotify, setWithSpotify ] = useState(!!data.spotify);
  const [ withApple, setWithApple ] = useState(!!data.apple);
  const [ description, setDescription ] = useState(data.description);
  const [ urlSpotify, setUrlSpotify ] = useState(data.spotify);
  const [ urlApple, setUrlApple ] = useState(data.apple);
    ...
    >
      ...
      >
        <Button
          ...
          onClick={save}
        >
          {label ? "Update label" : "Update artist"}
        </Button>
      </Flex>
    </Flex>
  );
...
export default EditArtistLabel;
